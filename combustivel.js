...
combustível gastos em uma viagem, ao utilizar um automóvel que faz
 ...
 Para isso, ele gostaria que você o auxiliasse através de um
  ...
  mesma (em km/h). Assim, pode-se obter distância percorrida e,
...
const [temp, veloc] = prompt("Tempo(h) velocidade(km/h):").split(" ").map(Number);
console.log((tempo * velocidade / 12).toFixed(3));

const tempo = +prompt("Tempo (horas):");
const velocidade = +prompt("Velocidade média (km/h):");
console.log((tempo * velocidade / 12).toFixed(3));
